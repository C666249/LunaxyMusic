...
import hashlib, os, shutil, subprocess, xml.etree.ElementTree as ET

stable = Path(__file__).resolve().parents[1]
beta = stable.with_name('LunaxyMusic-V92.8-Beta')
out = stable.parent / 'deliverables' / 'LunaxyMusic-V92.8'
...
for root in (stable,beta):
    for p in (root/'app/src/main').rglob('*.xml'):
        ET.parse(p)
    for required in ['WORKFLOW.md','DELIVERY_RULE.txt','PROJECT_STATE.md','RELEASE_NOTES_V92.8.md','APK_BUILD_STATUS.txt']:
        assert (root/required).is_file(), f'missing {required} in {root.name}'

assert 'applicationId = "com.xingyu.music"' in (stable/'app/build.gradle.kts').read_text()
assert 'versionName = "92.8.0"' in (stable/'app/build.gradle.kts').read_text()
assert 'applicationId = "com.xingyu.music.beta1"' in (beta/'app/build.gradle.kts').read_text()
assert 'versionName = "92.8.0-beta34"' in (beta/'app/build.gradle.kts').read_text()
...
for root in (stable,beta):
    entries, manifest=write_manifest(root)
    archive=out/(root.name+'.zip')
    cmd=['zip','-rq',str(archive),root.name,
         '-x','*/build/*','*/.gradle/*','*/.idea/*','*/.git/*','*/local.properties']
