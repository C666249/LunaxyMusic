...
beta = stable.with_name('LunaxyMusic-V92.8.3-Beta')
out = stable.parent / 'deliverables' / 'LunaxyMusic-V92.8.3'
...
for root in (stable,beta):
    ...
    for required in ['WORKFLOW.md','DELIVERY_RULE.txt','PROJECT_STATE.md','RELEASE_NOTES_V92.8.3.md','APK_BUILD_STATUS.txt','dev-logs/V92.8.3-validation.md']:
...
stack=(stable/'app/src/main/java/com/xingyu/music/ui/VinylStackView.java').read_text()
# Keep V92.8.2 timing customization intact.
assert 'PLAYER_OPEN_DURATION_MIN_MS = 450' in main
assert 'PLAYER_OPEN_DURATION_MAX_MS = 1000' in main
assert 'hero.setDuration(playerOpenMorphDurationMs)' in main
assert 'starStudioDurationSlider("播放器展开动画"' in main
# V92.8.3 single-owner Hero handoff.
assert 'nowVinylStack.setCenterRevealSuppressed(true)' in main
assert 'if (nowVinyl != null) nowVinyl.setAlpha(0f);' in main
assert 'morph.setAlpha(1f);' in main
assert 'nowVinylStack.setCenterRevealSuppressed(false)' in main
assert 'private boolean centerRevealSuppressed;' in stack
assert 'if (i == 2 && centerRevealSuppressed)' in stack
assert 'records[i].setAlpha(0f);' in stack
assert 'float vinylReveal =' not in main
...
assert 'versionCode = 948' in sgrad and 'versionName = "92.8.3"' in sgrad
assert 'applicationId = "com.xingyu.music.beta1"' in bgrad
assert 'versionCode = 949' in bgrad and 'versionName = "92.8.3-beta39"' in bgrad
assert 'LunaxyMusic-Stable-92.8.3-debug.apk' in sgrad and 'LunaxyMusic-Stable-92.8.3-release.apk' in sgrad
assert 'LunaxyMusic-Beta-92.8.3-beta39-debug.apk' in bgrad and 'LunaxyMusic-Beta-92.8.3-beta39-release.apk' in bgrad
