...
beta = stable.with_name('LunaxyMusic-V92.8.2-Beta')
out = stable.parent / 'deliverables' / 'LunaxyMusic-V92.8.2'
...
for root in (stable,beta):
    ...
    for required in ['WORKFLOW.md','DELIVERY_RULE.txt','PROJECT_STATE.md','RELEASE_NOTES_V92.8.2.md','APK_BUILD_STATUS.txt','dev-logs/V92.8.2-validation.md']:
...
assert 'PLAYER_OPEN_DURATION_DEFAULT_MS = 900' in main
assert 'player_open_duration_ms' in main
assert 'hero.setDuration(playerOpenMorphDurationMs)' in main
assert 'starStudioDurationSlider("播放器展开动画"' in main
assert 'scaledPlayerGestureSettleDuration(520L)' in main
assert 'scaledPlayerGestureSettleDuration(440L)' in main
...
assert 'versionCode = 946' in sgrad and 'versionName = "92.8.2"' in sgrad
assert 'applicationId = "com.xingyu.music.beta1"' in bgrad
assert 'versionCode = 947' in bgrad and 'versionName = "92.8.2-beta38"' in bgrad
assert 'LunaxyMusic-Stable-92.8.2-debug.apk' in sgrad and 'LunaxyMusic-Stable-92.8.2-release.apk' in sgrad
assert 'LunaxyMusic-Beta-92.8.2-beta38-debug.apk' in bgrad and 'LunaxyMusic-Beta-92.8.2-beta38-release.apk' in bgrad
